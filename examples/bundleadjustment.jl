using VisualGeometryDatasets, StaticArrays, Static, PaperViz
import NLLSsolver
...
function visualizeBALproblem(name)
    # Create the problem
    t = @elapsed begin
        data = loadbaldataset(name)
        problem = makeBALproblem(data)
        state = copy(problem.variables)
    end
    show(data)
    ...
    trajectory = PaperViz.Trajectory(state, result.trajectory, nothing, nothing)
    # Visualize the trajectory
    paperviz(trajectory, (X)->cost(X, problem.residuals))
end

visualizeBALproblem("problem-16-22106")